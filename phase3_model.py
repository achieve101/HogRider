...
class FeedbackFIRController(nn.Module):
    ...
    def oracle_forward(self, reference: torch.Tensor, expert_index: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        selected_filters = self.expert_filters[expert_index]
        padded = F.pad(reference, (self.fir_length - 1, 0)).unsqueeze(0)
        weights = torch.flip(selected_filters, dims=(-1,)).unsqueeze(1)
        selected = F.conv1d(padded, weights, groups=reference.shape[0]).squeeze(0)
