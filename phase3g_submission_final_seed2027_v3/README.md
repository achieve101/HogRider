# Phase 3G/4R 冻结权重生成式 FIR 控制器

本目录是赛题二正式推理包源码。模型采用反馈式逐采样接口，在时刻 `t` 接收
参考信号 `x[t]` 和上一采样点残差 `e[t-1]`。采样率固定为 48 kHz。

Phase 4R 只移除了包装器中逐采样重复进入的 PyTorch 推理上下文；权重、模型计算、
反馈延迟、FIR 更新时刻和输出安全检查均保持不变。
...
python DEEPANC_PARTICIPANT_KIT/validate_submission.py `
  --entry-point phase3g_submission_final_seed2027_v3.submission:create_model `
  --device cpu
