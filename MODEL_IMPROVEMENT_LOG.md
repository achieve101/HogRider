...
| 长训实验候选（未升级） | seed 2027，第 49 轮；`runs/phase3g_longtrain_seed2027_explore/P3G-E2/ep50/checkpoints/best_phase3g_selection.pt` |
...
Phase 3G 随后进行了 seed 2027 阶梯长训。E2 依次跑满 30、40、50 轮，最佳 D 位于
第 49 轮：开发 `S=21.4579/R=0.6525/D=21.2355`，最差路径 `4.0902 dB`。由于开发
反弹超过探索上限 `0.5447 dB`，探索判定未通过，自动后续没有运行旧模型续训对照或
三种子重跑。经用户后续明确授权，跳过三种子稳定性并直接测试固定的 epoch 49 候选：
官方公开闭环相对正式 epoch 15 主降噪提高 `2.7233 dB`，反弹降低 `0.0379 dB`；
路径 9/10 主降噪分别提高 `0.9855/0.0493 dB`，十路径主指标和综合分提高
`1.2385 dB/0.8171`。但十路径反弹由 `1.6870 dB` 增至 `1.8532 dB`，超过长训加强
上限 `1.7870 dB`，故不建议自动升级。正式 epoch 15 checkpoint、v3 与 ZIP 保持不变。
完整记录见 [PHASE3G_LONGTRAIN.md](PHASE3G_LONGTRAIN.md)。
...
### 2026-08-13：Phase 3G 阶梯长训完成，epoch 49 未升级

- seed 2027 从头完成 E1 5 轮和 E2 `30 → 40 → 50` 阶梯训练，最佳模型为 epoch 49；
- epoch 49 开发 `S=21.4579/R=0.6525/D=21.2355`，因反弹超过探索门槛而未自动进入后续；
- 用户决定跳过三种子稳定性；该协议偏差已写入直接最终评估候选锁；
- 官方公开闭环相对正式 epoch 15 主降噪 `+2.7233 dB`、反弹 `-0.0379 dB`；
- 路径 9/10 主降噪分别为 `16.2793/4.0268 dB`，均高于正式模型；
- 十路径 `S=19.1969/R=1.8532/C=12.8819`，原最终门槛通过，但加强反弹门槛失败；
- 未升级正式模型；7 个受保护文件的 SHA-256 全部不变。
